            ...
            if i>0 and numpy.all(image[i,j,:]==image[i-1,j,:]):
                ...
                if j<1919 and numpy.all(image[i,j,:]==image[i,j+1,:]):
                    ...
                    if j>1917 or numpy.all(image[i,j+1,:]==image[i,j+2,:]) or numpy.all(image[i,j+1,:]==image[i-1,j+1,:]):
                        ...
                        while numpy.any(image[i,j,:]!=image[i,j+1,:]) and numpy.any(image[i,j,:]!=image[i-1,j,:]) and j<1918:
                            n=n+1
...
## standard usb command function
    ...
    def gpıo0_a(self):
        self.command('w',0x00,0x1a,0x38,[int('00000000',2),int('00000011',2)])
        self.checkforerrors()

    def gpıo0_d(self):
        self.command('w', 0x00, 0x1a, 0x38, [int('00000000', 2), int('00000010', 2)])
        self.checkforerrors()
